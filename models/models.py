...
from .cifar import load_img_embeddings
...
def build_model(params, with_dis):
    ...
    """
    # source embeddings
    _src_emb, src_dico = load_img_embeddings(params, source=True)
    params.src_dico = src_dico
    src_emb = nn.Embedding(_src_emb.shape[0], _src_emb.shape[1], sparse=True)
    src_emb.weight.data.copy_(_src_emb)

    # target embeddings
    if params.tgt_lang:
        _tgt_emb, tgt_dico = load_img_embeddings(params, source=False)
        params.tgt_dico = tgt_dico
        tgt_emb = nn.Embedding(_tgt_emb.shape[0], _tgt_emb.shape[1], sparse=True)
